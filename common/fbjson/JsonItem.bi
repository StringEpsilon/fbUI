...
type jsonItem extends object
	...
	public:
		...
		declare function RemoveSelf() as boolean
end type
...
sub jsonItem.ParseObjectString(byref jsonString as string, startIndex as integer, endIndex as integer)
	...
	dim as parserState state = noToken
...
'~ function JsomItem.RemoveSelf() as boolean
	'~ if ( this.parent <> 0 ) then
		'~ dim as integer i
		'~ while i < this.parent->count and this.parent->children(i) <> @this
			'~ i += 1
		'~ wend 
